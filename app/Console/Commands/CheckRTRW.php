...
class CheckRTRW extends Command
{
    ...
    public function handle()
    {
        ...
        if (!empty($polygonPersil)) {
                ...
                try{
                    $area_not_clean = DB::connection('pgsql')->select(DB::raw("select st_area(st_intersects(st_transform(new_rtrw_disolve.geom,4326),polygon_persil.geom)) from polygon_persil, new_rtrw_disolve where polygon_persil.id = $polygonPersil->id"));
