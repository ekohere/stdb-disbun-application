<html>
    ...
    <body style="background-color: #FFFFFF;">
        <div class="page" id=halaman>
            <table border="1" >
                <tr>
                    <td style="padding: 25px; margin: 10px">
                        <img class="mt-1 mb-0 p-0 mr-0" width="100%" src="{{asset('image/kop_surat_stdb.jpg')}}"/>
                        <svg height="10" width="100%" class="mt-2 mb-2">
                            <line x1="100%" y1="0" style="stroke:rgb(0,0,0);stroke-width:10"></line>
                        </svg>
                        <h5 id="judul" class="m-0 p-0"><b>SURAT TANDA DAFTAR USAHA PERKEBUNAN UNTUK BUDIDAYA (STD-B)</b></h5>
                        <h5 id="judul" class="m-0 p-0">
                            <b>Desa {{$sTDBRegister->anggota->alamat_desa_ktp}}, Kecamatan {{$sTDBRegister->anggota->alamat_kec_ktp}} </b>
                        </h5>
                        <h5 id="judul" class="m-0 p-0"><b>Kabupaten Kutai Timur</b></h5>
{{--                        <svg height="5" width="100%" class="mt-2 mb-2">--}}
{{--                            <line x1="100%" y1="0" style="stroke:rgb(0,0,0);stroke-width:5"></line>--}}
{{--                        </svg>--}}
                        <h5 id="judul" class="m-0-1">Nomor: 525/{{ $sTDBRegister->no_surat<10?"00".$sTDBRegister->no_surat:($sTDBRegister->no_surat<100?"0".$sTDBRegister->no_surat:$sTDBRegister->no_surat) }}/DISBUN-KT/STD-B/{{$sTDBRegister->desa}} -
                            {{$sTDBRegister->kecamatan}}/{!! $bulan !!}/{{date_format($sTDBRegister->latest_status->pivot->created_at,'Y')}} </h5>
                        <br>
                        <div class="col-sm-12 pl-5 mt-3 isi_surat">
                            ...
                            @foreach($sTDBRegister->stdbDetailRegis as $key=>$item)
                                ...
                                <table class="ml-2 isi_surat">
                                    <tr>
                                        <td style="white-space: nowrap">Lokasi/Titik Koordinat Kebun</td>
                                        <td  width="15px">:</td>
                            ...
                            <div class="float-right col-sm-6 pl-5 mt-1 isi_surat">
                                <div class="mb-0" style="width: 100%; text-align: center;">
                                    Sangatta, {{$sTDBRegister->latest_status->pivot->created_at->isoFormat('D MMMM Y') }}
                                </div>
